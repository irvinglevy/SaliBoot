...
ReceivePacket(uint8_t *pui8Data, uint32_t *pui32Size)
{
   ...
   while(ui32Size == 0)
    ...
    //
    ui32Size -= 2;
    //HWREG(UART0_BASE + UART_O_DR) = ui32Size;
    //
  ...
  //  HWREG(UART0_BASE + UART_O_DR) = ui32Size;
    ...
    if(*pui32Size >= ui32Size)
    {
       //
        ...
        if(ui32Size == 9)		HWREG(GPIO_PORTC_BASE + (GPIO_O_DATA + (0x80 << 2))) = 0x80;  //IRVING
    ...
    {
		HWREG(GPIO_PORTC_BASE + (GPIO_O_DATA + (0x40 << 2))) = 0x40; //IRVING
       // NakPacket();
